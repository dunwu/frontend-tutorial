...
class NavPath extends React.PureComponent {
  ...
  componentWillMount() {
    ...
    const browseMenu = (item) => {
      ...
      console.debug('nameMap add entry: key=%s, value=%s', item.key, item.name);
      iconMap.set(item.key, item.icon);
      console.debug('iconMap add entry: key=%s, value=%s', item.key, item.icon);
  ...
  render() {
    ...
    for (const route of this.props.routes) {
      console.debug('path=%s, route=%o', route.path, route);
