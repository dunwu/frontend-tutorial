...
import { Icon, message, Spin, Tabs } from 'antd';
...
class App extends React.Component {
  ...
  async componentDidMount() {
    if (!this.props.login) {
      ...
      try {
        // 先去服务端验证下, 说不定已经登录了
        const res = await ajax.check();
        hide();

        // 注意这里, debug模式下每次刷新都必须重新登录
        if (res.code === 0 && res.data && !globalConfig.debug) {
  ...
  handleLoginError(errorMsg) {
    ...
    if (globalConfig.isSSO() && !globalConfig.debug) {
      // debug模式不支持调试单点登录
      // 因为没有单点登录的地址啊...跳不回来
      console.debug('not login, redirect to SSO login page');
      const redirect = encodeURIComponent(window.location.href);
      window.location.href = `${globalConfig.login.sso}${redirect}`;
    } else {
      message.error(errorMsg);
      console.debug('not login, redirect to Login component');
  ...
  onTabRemove = (targetKey) => {
    ...
    if (this.state.currentTabKey === targetKey) {
      ...
      if (currentTabIndex > 0) { // 如果当前tab左边还有tab, 就激活左边的
        nextTabKey = this.state.tabPanes[currentTabIndex - 1].key;
      } else if (currentTabIndex === 0 && this.state.tabPanes.length > 1) { // 否则就激活右边的tab
        nextTabKey = this.state.tabPanes[currentTabIndex + 1].key;
      }
  ...
  renderBody() {
    ...
    // 非tab模式, 显示面包屑和对应的组件
      ...
      return <div>
        <NavPath routes={this.props.routes}/>
  ...
  render() {
    ...
    if (this.state.tryingLogin) {
      return <div className="center-div"><Spin size="large"/></div>;
    }
